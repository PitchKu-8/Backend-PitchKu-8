...
import type { ConfirmOutlineRequest } from "./ai-engine.schema";
...
export async function generateOutlineHandler(
  ...
  const outline = await aiEngineService.generateOutline(
    userId,
 ...
 */
export async function confirmOutlineHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const userId = requireUserId(req);
  ...
  const confirmed = await aiEngineService.confirmOutline(
    userId,
    req.params.id as string,
    outline,
  );
...
export async function generateContentHandler(
  ...
  const userId = requireUserId(req);
  const deck = await aiEngineService.generateContent(
    userId,
    req.params.id as string,
  );
