...
export async function authMiddleware(
  ...
  if (!token) {
    ...
    throw new UnauthorizedError(
      "Authentication token not found or header format incorrect",
    );
  }
  ...
  if (error || !data.user) {
    log.warn(
      { action: "authMiddleware", error: error?.message },
  ...
  log.debug({ action: "authMiddleware", userId: req.userId }, "Token verified");
